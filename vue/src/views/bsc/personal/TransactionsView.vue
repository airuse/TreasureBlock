    ...
    <div class="bg-white shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">BSC交易历史</h1>
            <p class="mt-1 text-sm text-gray-500">查看和管理您的BSC交易记录</p>
          </div>
          <div class="flex items-center space-x-4">
            <!-- 网络状态 -->
          <div class="flex items-center space-x-2">
            <div class="w-3 h-3 bg-blue-500 rounded-full"></div>
            <span class="text-sm text-gray-600">BSC 网络</span>
          </div>
            <!-- 网络拥堵状态 -->
...
import CreateTransactionModal from '@/components/bsc/personal/CreateTransactionModal.vue'
...
// WebSocket相关
const { subscribeChainEvent } = useChainWebSocket('bsc')
// 收集本组件的取消订阅函数，避免重复回调
...
const createMinimalTransactionData = (tx: UserTransaction, fullData: any) => {
  ...
  const minimalData: any = {
    ...
    chainId: fullData.chain_id || (tx.chain === 'bsc' ? '56' : tx.chain),
    type: tx.chain, // 添加类型字段：bsc 或 btc
    ...
    value: tx.transaction_type === 'token' ? '0x0' : convertToHexString(tx.amount || '0'), // 代币转账value为0，BNB转账使用整数金额的十六进制格式
...
const loadTransactionStats = async () => {
  try {
    const response = await getUserTransactionStats({ chain: 'bsc' })
...
const loadGasRates = async () => {
  try {
    ...
    const response = await getGasRates({ chain: 'bsc' })
