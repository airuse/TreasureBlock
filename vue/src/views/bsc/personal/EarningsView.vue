    ...
    <div class="bg-white shadow rounded-lg">
      <div class="px-4 py-5 sm:p-6">
        <div class="flex items-center justify-between">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">BSC扫块收益</h1>
            <p class="mt-1 text-sm text-gray-500">查看和管理您的BSC扫块收益</p>
          </div>
...
const createEarningsChart = async () => {
  try {
    ...
    if (trendResponse.success) {
      const trendData = (trendResponse.data || []).filter((p: any) => p.source_chain === 'bsc')
...
const viewBlockDetails = (blockHeight: number) => {
  ...
  // 跳转到区块详情页面
  const route = `/bsc/blocks/${blockHeight}`
