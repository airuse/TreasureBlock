        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <div>
            ...
            <p class="mt-1 text-sm text-gray-900">
              <span class="font-medium">{{ formatMinerTip(block.miner_tip_eth) }} BNB</span>
              <span v-if="block.burned_eth && parseFloat(block.burned_eth) > 0" class="text-sm text-gray-500 ml-2">
                (燃烧: {{ formatBurnedEth(block.burned_eth) }} BNB)
              </span>
            </p>
          </div>
        </div>
              ...
              <div class="flex-1">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <span class="font-medium">{{ formatAmount(tx.amount || tx.value) }} BNB</span>
                  </div>
                  <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                      ...
                      <div class="md:col-span-2">
                        ...
                        <span class="text-gray-900 font-medium">
                          ...
                          {{ parsedResults[(tx.tx_id || tx.hash)][0]?.token_symbol || tx.token_symbol || 'BNB' }}
                        </span>
                      </div>
                    </div>
                  </div>
...
const formatNumber = {
  ...
  eth: (value: string | number | undefined): string => {
    ...
    try {
      ...
      console.error('BNB格式化错误:', error, value)
      return '0'
    }
  },
  ...
  weiToEth: (value: string | number | undefined): string => {
    ...
    try {
      ...
      console.error('Wei到BNB转换错误:', error, value)
      return '0'
    }
  }
}
...
const formatBurnedFee = (baseFee: string | number | undefined, gasUsed: number): string => {
  if (!baseFee || !gasUsed) return '0 BNB'
  ...
  try {
    ...
    // 使用统一格式化工具，转换为BNB并添加单位
    return `${formatNumber.weiToEth(burnedWei.toString())} BNB`
  } catch (error) {
...
const formatSavedFee = (tx: any, baseFee: string | number | undefined, gasUsed: number): string => {
  ...
  try {
    ...
    if (!receipt?.effective_gas_price) {
      ...
      return `${formatNumber.weiToEth(savedWei.toString())} BNB`
    }
...
const formatTransactionFeeFromReceipt = (receipt: any): string => {
  ...
  if (fee === 0) return '0 BNB'
  
  // 使用统一格式化工具，转换为BNB并添加单位
  return `${formatNumber.weiToEth(fee)} BNB`
}
