          ...
          <div class="px-6 py-4 flex-1 overflow-y-auto">
            ...
            <div>
              ...
              <div class="flex space-x-4">
                <label class="flex items-center">
                  ...
                  />
                  <span class="text-sm text-gray-700">BNB转账</span>
                </label>
                <label class="flex items-center">
                  <input
                    type="radio"
                    ...
                    @change="handleTransactionTypeChange"
                  />
                  <span class="text-sm text-gray-700">BEP-20代币</span>
                </label>
              </div>
            </div>

            <!-- 合约操作类型选择 (仅BEP-20时显示) -->
            <div v-if="transactionType === 'bep20'">
            ...
            <div v-if="shouldShowAmount">
              ...
              <div class="mt-1 text-sm text-gray-500">
                <span v-if="transactionType === 'bnb'">{{ formatToWei(form.amount) }} wei</span>
...
// 交易类型
const transactionType = ref<'bnb' | 'bep20'>('bnb')
...
// 代币相关
const tokens = ref<BEP20Token[]>([])
const selectedTokenSearch = ref('')
const showTokenDropdown = ref(false)
const selectedToken = ref<BEP20Token | null>(null)
...
const shouldShowFromAddress = computed(() => {
  if (transactionType.value === 'bnb') return true
...
const getFromAddressLabel = () => {
  if (transactionType.value === 'bnb') return '发送地址'
...
const getFromAddressPlaceholder = () => {
  if (transactionType.value === 'bnb') return '选择或输入发送地址'
  switch (contractOperationType.value) {
...
const getToAddressLabel = () => {
  if (transactionType.value === 'bnb') return '接收地址'
  switch (contractOperationType.value) {
...
const getToAddressPlaceholder = () => {
  if (transactionType.value === 'bnb') return '选择或输入接收地址'
  switch (contractOperationType.value) {
...
const getAmountLabel = () => {
  if (transactionType.value === 'bnb') return '交易金额'
  switch (contractOperationType.value) {
...
const getAmountPlaceholder = () => {
  if (transactionType.value === 'bnb') return '0.0'
  switch (contractOperationType.value) {
...
const loadAddresses = async () => {
  try {
    const response = await getPersonalAddresses("bsc")
    if (response.success) {
...
const loadTokens = async () => {
  try {
    ...
    tokens.value = [
      {
        contract_address: '0x55d398326f99059fF775485246999027B3197955', // USDT on BSC
        name: 'Tether USD',
        symbol: 'USDT',
        decimals: 18,
        balance: '0'
      ...
      {
        contract_address: '0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d', // USDC on BSC
        name: 'USD Coin',
        symbol: 'USDC',
        decimals: 18,
        balance: '0'
...
const handleTransactionTypeChange = () => {
  if (transactionType.value === 'bnb') {
    form.value.symbol = 'BNB'
...
// 选择代币
const selectToken = (token: BEP20Token) => {
  selectedToken.value = token
...
// 格式化代币余额
const formatTokenBalance = (token: BEP20Token) => {
...
const handleSubmit = async () => {
  try {
    ...
    if (transactionType.value === 'bep20' && !selectedToken.value) {
      alert('请选择代币')
      return
    }
    ...
    const submitData = {
      ...form.value,
      // 添加交易类型和合约操作类型
      transaction_type: transactionType.value === 'bnb' ? 'coin' : 'token',
      contract_operation_type: contractOperationType.value,
      // 如果是代币交易，添加代币合约地址
      token_contract_address: selectedToken.value?.contract_address || '',
      // 确保金额为整数格式
      amount: form.value.amount ? (transactionType.value === 'bnb' ? formatToWei(form.value.amount) : formatToTokenUnits(form.value.amount, selectedToken.value?.decimals || 18)) : '0',
      // 确保手续费为整数格式
